...
//
// Gated by the `otlp` build tag. Without the tag this file is the only
// one that compiles — the package exists but registers nothing — so
// default `go build` produces a binary with no OTLP dependency baked
// in. Build with `-tags=otlp` to include it.
package otlp
