// Package scope provides permission scope types, hierarchy, and validation
// for the GoCache plugin system.
//
...
func Implies(have, need Scope) bool {
	...
	switch have {
	case ScopeAdmin:
		return need == ScopeWrite || need == ScopeRead || need == ScopeEvents || isServerQueryScope(need)
	case ScopeWrite:
