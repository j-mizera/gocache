# Phase 1 hybrid encoding — `redis-benchmark` vs Phase 0 baseline

**Branch:** `feat/slab-phase0-resp-pool`
**Commit:** `34b28d6` (Stage 6 — zset dual-path)
**Date:** 2026-04-20

## Setup

Same containerized harness as Phase 0. Target/client share a bridge network
...
- Suite: `ping_inline,ping_mbulk,set,get,incr,lpush,rpush,lpop,rpop,sadd,hset,lrange_100,mset`
  — `spop` excluded as in Phase 0 (known gocache stall).

## Standard suite (no pipelining)

Raw CSVs: [`phase-0-gocache.csv`](phase-0-gocache.csv) vs
[`phase-1-gocache.csv`](phase-1-gocache.csv).

| command | phase-0 rps | phase-1 rps | Δ rps% | phase-0 p99 | phase-1 p99 | Δ p99% |
|---|---:|---:|---:|---:|---:|---:|
...
**The catastrophic regression of the first Phase-1 attempt is gone.** Old
Phase 1 measured: HSET -77%, LPUSH -50%, LRANGE_100 -98%. New Phase 1: HSET
...
ListAppendRight/SetAdd to preallocate (e.g. 64 B initial cap) is a Phase-1
follow-up.

## Pipelined suite (P=10)

Raw CSVs: [`phase-0-gocache-pipelined.csv`](phase-0-gocache-pipelined.csv)
vs [`phase-1-gocache-pipelined.csv`](phase-1-gocache-pipelined.csv).

| command | phase-0 rps | phase-1 rps | Δ rps% |
|---|---:|---:|---:|
...
Worth profiling in Phase 2 or addressing via:
...
- The core Phase-1 thesis claim — that a hybrid encoding is required, not
...
These are Phase-2-aware concerns: the slab allocator replaces the
